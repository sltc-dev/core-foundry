...
**位置**：优先保存在项目根目录已有的文档目录中（如 `docs/` 或 `doc/`）。

- 如果 `docs/` 已存在，则**必须**使用它，不要另建 `doc/`。
- 如果两个都不存在，建议使用 `docs/`。
- **禁止**保存在业务代码文件夹内。
...
5. 保存在项目根目录的 `docs/payment/` 目录中（严禁放在代码文件夹 `payment/` 内）
```
...
- 每个主要功能/模块创建一个实现文档
- **目录优先规则**：优先使用项目现有的文档目录（`docs/` 或 `doc/`）。如果已有 `docs/`，严禁再创建 `doc/`。
- **存放路径**：`docs/功能名/IMPLEMENTATION.md` 或 `doc/功能名/IMPLEMENTATION.md`
- **严禁**将文档直接放置在源码文件夹（如 `.py`, `.js`, `.go` 目录）中
- 在相关文档文件之间建立链接
